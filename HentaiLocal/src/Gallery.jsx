// Gallery.jsx - Modified gallery with AI naming support
/* global SillyTavern */
...
/**
 * Settings Panel Component with AI options
 */
function SettingsPanel({ characterName, settings, customPrompts, assignments, onSave, onAddPrompt, onEditPrompt, onDeletePrompt }) {
  ...
  const [aiEnabled, setAiEnabled] = useState(settings.aiNaming?.enabled || false);
  const [aiProvider, setAiProvider] = useState(settings.aiNaming?.provider || 'openai');
  const [aiApiKey, setAiApiKey] = useState(settings.aiNaming?.apiKey || '');
  ...
  return (
    <div className="local-image-settings">
      ...
      {/* AI Naming Settings */}
      <div className="settings-row settings-divider">
        <h4>AI Naming</h4>
      </div>
      <div className="settings-row">
        <label className="settings-checkbox">
          ...
          />
          <span>Enable AI-powered naming</span>
        </label>
      </div>
      {aiEnabled && (
        <>
          ...
          <div className="settings-row">
            <button className="settings-btn settings-btn-primary" onClick={handleAiSave}>
              Save AI Settings
            </button>
          </div>
        </>
...
/**
 * Gallery Modal Component with AI naming buttons
 */
function Gallery({ characterName, onClose, assignments, characterSettings, customPrompts, onAssign, onUnassign, onUpdateDescription, onSaveSettings, onAddPrompt, onEditPrompt, onDeletePrompt, isPersona = false }) {
  ...
  const [aiNamingInProgress, setAiNamingInProgress] = useState(null);
  ...
  const handleDelete = useCallback(async (image) => {
    ...
    try {
      ...
      if (assignmentName) onUnassign(assignmentName);
      fetchImages();
    } catch (e) {
  ...
  const handleAssign = useCallback((image) => {
    ...
    if (name && name.trim()) {
      ...
      if (currentName && currentName !== name) onUnassign(currentName);
  ...
  const handleEditDescription = useCallback((image) => {
    ...
    if (description !== null) onUpdateDescription(currentName, description.trim());
  ...
  const handleAINaming = useCallback(async (image) => {
    if (!AINaming.isAvailable()) return;
    setAiNamingInProgress(image.src);
    try {
      const currentName = Object.keys(assignments || {}).find(key => {
        const val = assignments[key];
        const path = typeof val === 'string' ? val : val?.path;
      ...
      const [name, description] = await Promise.all([
        AINaming.generateName([], characterName, 'descriptive'),
        AINaming.generateDescription([], characterName)
      ]);
      if (currentName && currentName !== name) onUnassign(currentName);
      onAssign(name, image.src, description);
    } catch (e) {
      console.error('[Gallery] AI naming failed:', e);
      setError('AI naming failed: ' + e.message);
    } finally {
      setAiNamingInProgress(null);
    }
  }, [assignments, onAssign, onUnassign]);

  const handleBatchAINaming = useCallback(async () => {
    if (!AINaming.isAvailable()) return;
    const unassigned = images.filter(img => !assignments || !Object.values(assignments).some(a => {
      const path = typeof a === 'string' ? a : a?.path;
      return path === img.src;
    }));
    if (unassigned.length === 0) {
      alert('No unassigned images to process.');
      return;
    }
    setAiNamingInProgress('batch');
    try {
      ...
      console.error('[Gallery] Batch AI naming failed:', e);
      setError('Batch AI naming failed: ' + e.message);
    } finally {
      setAiNamingInProgress(null);
    }
  }, [images, assignments, onAssign, fetchImages]);
  ...
  const handleImageClick = useCallback((e, image) => {
    ...
    setOptionsMenu({ image, position: { x, y } });
  }, []);
    ...
    <div className="local-image-modal-backdrop" onClick={handleBackdropClick} onDrop={handleDrop} onDragOver={handleDragOver}>
      <div className="local-image-modal" onClick={() => setOptionsMenu(null)}>
        <div className="local-image-modal-header">
          ...
          <div className="local-image-modal-actions">
            <button className={`menu_button ${showScraper ? 'menu_button_active' : ''}`} onClick={() => setShowScraper(true)} title="Booru Scraper">
            ...
            <button className="menu_button" onClick={() => fileInputRef.current?.click()} disabled={uploading} title="Upload Images">
        ...
        {showSettings && <SettingsPanel characterName={characterName} settings={characterSettings} customPrompts={customPrompts} assignments={assignments} onSave={onSaveSettings} onAddPrompt={onAddPrompt} onEditPrompt={onEditPrompt} onDeletePrompt={onDeletePrompt} />}
        ...
        <div className="local-image-info">
          ...
          {assignedCount > 0 && <span className="assigned-count">{assignedCount} image{assignedCount !== 1 ? 's' : ''} assigned</span>}
          {AINaming.isAvailable() && (
            <button className="ai-naming-btn" onClick={handleBatchAINaming} disabled={aiNamingInProgress === 'batch'} title="Batch AI naming">
              {aiNamingInProgress === 'batch' ? <i className="fa-solid fa-spinner fa-spin"></i> : <i className="fa-solid fa-robot"></i>}
              Batch AI Name
            </button>
          )}
        </div>
        <div className="local-image-modal-content">
          {loading ? <div className="local-image-loading">Loading...</div> : images.length === 0 ? (
            <div className="local-image-empty"><p>No images yet</p><p>Drag and drop images here or click upload</p></div>
          ) : (
            <div className="local-image-grid">
              {images.map((image, index) => {
                const { name: assignmentName, description } = getAssignmentInfo(image.src);
                return (
                  <div key={index} className={`local-image-item ${assignmentName ? 'assigned' : ''}`} onClick={(e) => handleImageClick(e, image)} title={description || assignmentName || image.name}>
                    ...
                    {AINaming.isAvailable() && !assignmentName && (
                      <button className="ai-name-btn" onClick={(e) => { e.stopPropagation(); handleAINaming(image); }} disabled={aiNamingInProgress === image.src} title="Generate AI name">
                        {aiNamingInProgress === image.src ? <i className="fa-solid fa-spinner fa-spin"></i> : <i className="fa-solid fa-wand-magic-sparkles"></i>}
                      </button>
                    )}
                  </div>
                );
      ...
      {optionsMenu && (
        <ImageOptionsMenu
          ...
          onRename={() => { const name = getAssignmentInfo(optionsMenu.image.src).name; if (name) onUnassign(name); }}
      ...
      {showScraper && (
        <ScraperPanel
          ...
          onImageSave={(name, path, description) => { onAssign(name, path, description); fetchImages(); }}
        />
      )}
    </div>
  );
