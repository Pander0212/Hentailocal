 ...
 */
function ImageOptionsMenu({ image, position, onClose, onView, onAssign, onEditDescription, onRename, onDelete, onAINaming, assignmentName, assignmentDescription, showAINaming }) {
    ...
    return (
        ...
        >
            ...
            {showAINaming && (
                <div className="options-menu-item options-menu-item-ai" onClick={() => { onAINaming(); onClose(); }}>
                    <i className="fa-solid fa-wand-magic-sparkles"></i> AI Generate Name
                </div>
            )}
            {assignmentName && (
...
/**
 * Settings Panel Component - enhanced with prompt injection
 */
...
/**
 * Gallery Modal Component - Enhanced with AI naming
 */
function Gallery({ characterName, onClose, assignments, characterSettings, customPrompts, onAssign, onUnassign, onUpdateDescription, onSaveSettings, onAddPrompt, onEditPrompt, onDeletePrompt, isPersona = false, onAINameImage }) {
    ...
    const handleAINaming = useCallback(async (image) => {
        if (!onAINameImage) return;
        setAiNamingInProgress(image.src);
        try {
            await onAINameImage(image.src, characterName);
            // Re-render the gallery will happen via parent re-render
        } catch (e) {
            setError(`AI naming failed: ${e.message}`);
        } finally {
            setAiNamingInProgress(null);
        }
    }, [onAINameImage, characterName]);
        ...
        >
                    ...
                    {loading ? (
                        ...
                        <div className="local-image-grid">
                            {images.map((image, index) => {
                                ...
                                const isNaming = aiNamingInProgress === image.src;
                                return (
                                    <div
                                        key={index}
                                        className={`local-image-item ${assignmentName ? 'assigned' : ''} ${isNaming ? 'ai-naming' : ''}`}
                                        onClick={(e) => handleImageClick(e, image)}
                                        title={description || assignmentName || image.name}
                                    >
                                        <img src={image.src} alt={image.name} loading="lazy" />
                                        {isNaming && (
                                            <div className="ai-naming-overlay">
            ...
            {optionsMenu && (
                <ImageOptionsMenu
                    ...
                    onAIName={() => handleAINaming(optionsMenu.image)}
                    onRename={() => {
